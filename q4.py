...
def main():
  ...
  server_down = int(os.getenv("SERVER_DOWN"))
  ...
  for item in inputs:
    ...
    if len(result_lists):
      for ip in result_lists:
        ...
        if t_data[1] == data[1] and data[2][0] != '-':
          ...
          if data[1] in down_lists:
            recovered_lists.append([data[1],start,stop,result])
            down_lists.remove(data[1])
          elif data[1] in ip_lists:
            recovered_lists.append([data[1],start,stop,result])
    ...
    if data[2].strip('\n') == '-':
      ...
      if len(timeout_lists[data[1]]) >= server_down:
        if data[1] in down_lists:
          continue
        print(data[1],"SERVER DOWN at",data[0])
        result_lists[data[1]] = timeout_lists[data[1]][-1]
        down_lists.append(data[1])
        if data[1] in ip_lists:
          ip_lists.remove(data[1])
      elif len(timeout_lists[data[1]]) >= num_error:
