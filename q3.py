...
def main():
  ...
  count_avg = int(os.getenv("COUNT_AVERAGE"))
  max_timeout = (int(os.getenv("MAX_TIMEOUT"))/1000)
  ...
  for item in inputs:
    ...
    if data[2].strip('\n') == '-':
      ...
      try:
        ...
        if len(time_lists[data[1]]):
          time_lists[data[1]].pop(0)
      ...
      time_lists[data[1]].append(data[2].strip('\n'))

      if len(time_lists[data[1]]) >= count_avg:
        total = 0
        for item in time_lists[data[1]]:
          total += int(item)

        if total/count_avg >= max_timeout:
          try:
            timeout_lists[data[1]]
          except:
            timeout_lists[data[1]] = []
          ...
          if data[1] not in ip_lists:
            print(data[1],"HEAVY LOAD at",data[0])
            ip_lists.append(data[1])
  ...
  if len(timeout_lists) > 0:
    for ip in timeout_lists:
      if len(timeout_lists[ip]) >= num_error:
        print(ip,"not recovered since",timeout_lists[ip][0][0])
